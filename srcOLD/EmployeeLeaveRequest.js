	...
	constructor() {
		...
		componentDidMount() {
            ...
            $.ajax({
                ...
                success: function(data,textStatus,jqXHR)
                {
                    ...
                    if(data.length!=0){
                    var tab='<thead><tr class="headcolor"><th>Id</th><th>Name</th><th>LeaveName</th><th>#Days</th><th>Date</th><th>From</th><th>To</th><th>Subject</th><th colspan="2"  style="text-align:center;">Actions</th></tr></thead>';
                  
                  
                    $.each(data, function (i,item) {
                      /* alert(item);
                     */ tab += '<tr class="success" ><td>' + item.employeeId + '</td><td>' + item.employeeName + '</td><td>'+item.leaveType+'</td><td>' + item.day + '</td><td>'+item.date+'</td><td>' + item.fromDate + '</td><td>'  + item.toDate  +'</td><td>' + item.subject + '</td><td><button class="AcceptSelect"> Accept</button></td><td><button class="RejectSelect"> Reject</button></td></tr>';
        ...
        $(document).ready(function(){
            ...
            $("#tableHeadings").on('click','.AcceptSelect',function(){
                 ...
                 self.state.leaveType=currentRow.find("td:eq(2)").text();
                 self.state.noOfDays=currentRow.find("td:eq(3)").text(); // get current row 2nd TD
                 self.state.date=currentRow.find("td:eq(4)").text();
                 self.state.fromDate=currentRow.find("td:eq(5)").text();
                 self.state.toDate=currentRow.find("td:eq(6)").text(); // get current row 3rd TD
                 self.state.subject=currentRow.find("td:eq(7)").text(); // get current row 3rd TD
